...
cyclic_2_permutation = (
    np.asarray([[1, 0, 0], [0, 1, 0], [0, 0, 1]]),  # I
    np.asarray([[-1, 0, 0], [0, -1, 0], [0, 0, 1]]),  # 180 deg [001]
)


orthorhombic_permutation = (
    np.asarray([[1, 0, 0], [0, 1, 0], [0, 0, 1]]),  # I
    np.asarray([[1, 0, 0], [0, -1, 0], [0, 0, -1]]),  # 180 deg [100]
    np.asarray([[-1, 0, 0], [0, 1, 0], [0, 0, -1]]),  # 180 deg [010]
    np.asarray([[-1, 0, 0], [0, -1, 0], [0, 0, 1]]),  # 180 deg [001]
)


trigonal_permutation = (
        np.asarray([[ 1,  0,  0], [ 0,  1,  0], [ 0,  0,  1]]),
        np.asarray([[ 0,  1,  0], [-1, -1,  0], [ 0,  0,  1]]),
        np.asarray([[-1, -1,  0], [ 1,  0,  0], [ 0,  0,  1]]),
        np.asarray([[ 1,  0,  0], [-1, -1,  0], [ 0,  0, -1]]),
        np.asarray([[ 0,  1,  0], [ 1,  0,  0], [ 0,  0, -1]]),
        np.asarray([[-1, -1,  0], [ 0,  1,  0], [ 0,  0, -1]]),
)

tetragonal_permutation = (
    np.asarray([[1, 0, 0], [0, 1, 0], [0, 0, 1]]),  # I
    np.asarray([[0, -1, 0], [1, 0, 0], [0, 0, 1]]),  # 90 deg [001]
    np.asarray([[-1, 0, 0], [0, -1, 0], [0, 0, 1]]),  # 180 deg [001]
    np.asarray([[0, 1, 0], [-1, 0, 0], [0, 0, 1]]),  # 270 deg [001]

    np.asarray([[1, 0, 0], [0, -1, 0], [0, 0, -1]]),  # 180 deg [100]
    np.asarray([[0, 1, 0], [1, 0, 0], [0, 0, -1]]),  # 180 deg [110]
    np.asarray([[-1, 0, 0], [0, 1, 0], [0, 0, -1]]),  # 180 deg [010]
    np.asarray([[0, -1, 0], [-1, 0, 0], [0, 0, -1]]),  # 180 deg [1-10]
)

hexagonal_permutation = (
        np.asarray([[ 1,  0,  0], [ 0,  1,  0], [ 0,  0,  1]]),
        np.asarray([[ 1,  1,  0], [-1,  0,  0], [ 0,  0,  1]]),
        np.asarray([[ 0,  1,  0], [-1, -1,  0], [ 0,  0,  1]]),
        np.asarray([[-1,  0,  0], [ 0, -1,  0], [ 0,  0,  1]]),
...
tetrahedral_permutation = (
    ...
    np.asarray([[0, 1, 0], [0, 0, 1], [1, 0, 0]]),  # 120 deg [111]
    np.asarray([[0, -1, 0], [0, 0, 1], [-1, 0, 0]]),  # 120 deg [11-1]
    np.asarray([[0, 1, 0], [0, 0, -1], [-1, 0, 0]]),  # 120 deg [1-11]
    np.asarray([[0, -1, 0], [0, 0, -1], [1, 0, 0]]),  # 120 deg [-111]
    np.asarray([[0, 0, -1], [1, 0, 0], [0, -1, 0]]),  # 120 deg [1-1-1]
    np.asarray([[0, 0, -1], [-1, 0, 0], [0, 1, 0]]),  # 120 deg [-11-1]
    np.asarray([[0, 0, 1], [-1, 0, 0], [0, -1, 0]]),  # 120 deg [-1-11]
    np.asarray([[0, 0, 1], [1, 0, 0], [0, 1, 0]]),  # 120 deg [-1-1-1]
)
...
cubic_permutation = (
    ...
    np.asarray([[1, 0, 0], [0, 0, -1], [0, 1, 0]]),  # 90 deg [100]
    np.asarray([[1, 0, 0], [0, -1, 0], [0, 0, -1]]),  # 180 deg [100]
    np.asarray([[1, 0, 0], [0, 0, 1], [0, -1, 0]]),  # 270 deg [100]

    np.asarray([[0, 0, 1], [0, 1, 0], [-1, 0, 0]]),  # 90 deg [010]
    np.asarray([[-1, 0, 0], [0, 1, 0], [0, 0, -1]]),  # 180 deg [010]
    np.asarray([[0, 0, -1], [0, 1, 0], [1, 0, 0]]),  # 270 deg [010]
    ...
    np.asarray([[0, 1, 0], [1, 0, 0], [0, 0, -1]]),  # 180 deg [110]
    np.asarray([[0, 0, 1], [0, -1, 0], [1, 0, 0]]),  # 180 deg [101]
    np.asarray([[-1, 0, 0], [0, 0, 1], [0, 1, 0]]),  # 180 deg [011]

    np.asarray([[0, -1, 0], [-1, 0, 0], [0, 0, -1]]),  # 180 deg [1-10]
    np.asarray([[0, 0, -1], [0, -1, 0], [-1, 0, 0]]),  # 180 deg [10-1]
    np.asarray([[-1, 0, 0], [0, 0, -1], [0, -1, 0]]),  # 180 deg [01-1]
...
octahedral_permutation = cubic_permutation